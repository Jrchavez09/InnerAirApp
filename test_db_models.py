...
from inner_air.models import User, Exercise, Routine, Statistics, Favorites, Category, UserRating
...
def test_routine_model(db_session):
    ...
    assert test_routine.user_id == db.session.query(User).first().id
...
def test_favorites_model(db_session):
    ...
    db.session.commit()
    assert test_favorites_add.user_id == db.session.query(User).first().id
...
def test_userRating_model_new(db_session, current_user, current_exercise):
    ...
    if db_session.query(UserRating).filter_by(user_id=current_user.id,
        ...
        db.session.add(
            UserRating(user_rating=100, user_id=current_user.id, exercise_id=current_exercise.exerciseID))
        db.session.commit()
    else:
